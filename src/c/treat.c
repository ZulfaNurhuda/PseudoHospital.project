...
boolean treatPatient(Hospital *hospital, Session *session, const char *patientUsername)
{
    ...
    {
        printSuccess("Pasien tidak memiliki penyakit yang terdeteksi!");
        patient->treatedStatus = true; // Set treatedStatus to true even if no disease is detected
        return true;
    } else if (patient->disease[0] == '\0')
    {
        printError("Penyakit pasien tidak terdeteksi!");
        return false;
    }
