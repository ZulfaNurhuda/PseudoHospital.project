...
boolean treatPatient(Hospital *hospital, Session *session)
{
    ...
    // Memeriksa pasien yang berada di antrian ruangan dokter, mengambil pasien pertama
    int patientId = doctorRoom->patientInRoom.patientId[0]; // Mengambil ID pasien pertama dalam ruangan
    int patientIdx = -1;
    ...
    // Jika pasien sudah berada di depan antrian (ID pertama dalam patientInRoom)
    ...
    // Memeriksa resep obat untuk penyakit pasien
    ...
    if (medicationCount > 0)
    {
        ...
        // Mengurutkan berdasarkan urutan dosis obat
    ...
    if (prescribedMedications.nEff > 0)
    {
        ...
        for (int i = 0; i < prescribedMedications.nEff; i++)
        {
            ...
            {
                printf("Obat tidak ditemukan untuk ID: %d\n", prescribedMedications.elements[i].id);
            }
        }
